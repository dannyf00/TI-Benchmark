#include "TIBnchMk.h"			//ti benchmark

//helper applications / user API
//performing the integer portion of the benchmark
unsigned long TIBnchMki(void) {
	unsigned long clock_start, clock_end;
	unsigned long idx;

	clock_start=clock();				//take the starting clock
	//8-bit integer
	idx=iLOOPS_8; while (idx--) {		//starting benchmark
		main_8();						//arithmetic
		main_8m();						//matrix
		main_8sw();						//switch case
	}

	//16-bit integer
	idx=iLOOPS_16; while (idx--) {		//starting benchmark
		main_16();						//arithmetic
		main_16m();						//matrix
		main_16sw();					//switch case
		main_mmlt16();					//matrix
	}

	//32-bit integer
	idx=iLOOPS_32; while (idx--) {		//starting benchmark
		main_32();						//arithmetic
		main_mmlt32();					//matrix
	}
...
//performing the floating point portion of the benchmark
unsigned long TIBnchMkf(void) {
	unsigned long clock_start, clock_end;
	unsigned long idx;

	clock_start=clock();				//take the starting clock
	//floating point FIR
	idx=fLOOPS; while (idx--) {			//starting benchmark
		main_f();						//floating point arithmetics
		main_fir();						//floating point FIR
	}
	clock_end=clock();					//take the ending clock
...
*******************************************************************************/
//typedef unsigned short UInt16;
const UInt32 m1_mmlt32[3][4] = {
    {0x01, 0x02, 0x03, 0x04},
    {0x05, 0x06, 0x07, 0x08},
    {0x09, 0x0A, 0x0B, 0x0C}
};
const UInt32 m2_mmlt32[4][5] = {
    {0x01, 0x02, 0x03, 0x04, 0x05},
    {0x06, 0x07, 0x08, 0x09, 0x0A},
    {0x0B, 0x0C, 0x0D, 0x0E, 0x0F},
    {0x10, 0x11, 0x12, 0x13, 0x14}
...
void main_mmlt32(void) {
    ...
    for(m = 0; m < 3; m++) {
        for(p = 0; p < 5; p++) {
            m3[m][p] = 0;
            for(n = 0; n < 4; n++) {
                m3[m][p] += m1_mmlt32[m][n] * m2_mmlt32[n][p];
            }
        }
    }
    return;
...
void main_fir(void) {
    ...
    volatile float iOUTPUT[36],sum;
    for(y = 0; y < 36; y++) {
        sum=0;
        for(i = 0; i < FIR_LENGTH/2; i++) {
            sum = sum+COEFF[i] * ( iINPUT[y + 16 - i] + iINPUT[y + i] );
        }
        iOUTPUT[y] = sum + (iINPUT[y + FIR_LENGTH/2] * COEFF[FIR_LENGTH/2] );
    }
    return;
}
